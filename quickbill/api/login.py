...
@frappe.whitelist()
def get_login_details(company=None):
	"""Get current user's login details, dashboard data, and related lookups.

	Args:
		company: Optional company filter for financial totals

	Returns:
		dict matching the login_details schema
	"""
	...
	total_sales = _get_total_sales(user, sales_person, target_company)
	total_outstanding = _get_total_outstanding(user, sales_person, target_company)
	total_expenses = _get_total_expenses(employee_id, target_company)
	total_paid = _get_total_paid(user, sales_person, target_company)
